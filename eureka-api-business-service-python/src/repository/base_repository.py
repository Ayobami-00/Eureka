...
class BaseRepository:
    ...
    def read_by_options(self, schema, context, eager=False):
        with self.session_factory() as session:
            schema_as_dict = schema.dict(exclude_none=True)
            ordering = schema_as_dict.get("ordering", constants.ORDERING)
            order_query = (
    ...
    def read_by_id(self, id: int, context, eager=False):
        with self.session_factory() as session:
            query = session.query(self.model)
            if eager:
                for eager in getattr(self.model, "eagers", []):
